document.addEventListener('DOMContentLoaded', function () {
  ...
  function serviceOptions() {
    return ['Catering', 'Decoração', 'Som e iluminação', 'Projetor e ecrã', 'Mesas e cadeiras', 'Climatização', 'Estacionamento', 'Segurança', 'Limpeza', 'Internet Wi-Fi'].map(function (service) {
      return '<label class="service-choice"><input type="checkbox" name="interestServices" value="' + esc(service) + '"><span>' + esc(service) + '</span></label>';
    }).join('');
  }
  ...
  document.getElementById('metricAll').textContent = requests.length;
  document.getElementById('metricPending').textContent = requests.filter(function (r) { return r.status === 'pending'; }).length;
  document.getElementById('metricConfirmed').textContent = requests.filter(function (r) { return r.status === 'confirmed'; }).length;
  document.getElementById('metricMessages').textContent = requests.reduce(function (sum, r) { return sum + (r.messages || []).length; }, 0);
  document.getElementById('clientVisitCount').textContent = requests.length;

  function renderList() {
    var list = document.getElementById('clientVisitList');
    if (!requests.length) { list.innerHTML = '<p class="list-empty">Ainda não tem visitas. Explore os espaços e agende a primeira.</p>'; return; }
    list.innerHTML = requests.map(function (r) { return '<button class="visit-item" type="button" data-id="' + r.id + '" aria-current="' + (r.id === selectedId) + '"><span class="avatar"><i class="bx bx-building-house"></i></span><span class="visit-item__copy"><strong>' + esc(r.venue) + '</strong><span>Pedido ' + esc(r.id) + '</span><small><i class="bx bx-calendar"></i> ' + date(r.date) + ' · ' + esc(r.time) + '</small></span><span class="visit-item__side">' + requestBadge(r) + '</span></button>'; }).join('');
  ...
  function renderDetail() {
    ...
    detail.innerHTML = '<button class="btn btn--outline visit-detail-back" id="closeVisitDetail" type="button"><i class="bx bx-arrow-back"></i> Voltar à lista</button><div class="detail-head"><div><span class="eyebrow">' + esc(r.id) + '</span><h3>' + esc(r.venue) + '</h3><p>Solicitação enviada ao proprietário</p></div>' + requestBadge(r) + '</div>' +
      ...
      '<form class="inline-form" id="interestForm"><input id="interestType" placeholder="Tipo de evento" required><input id="interestDate" type="date" required><p class="availability-error" id="eventAvailabilityError" role="alert" hidden></p><input id="interestGuests" type="number" min="1" placeholder="Número de convidados" required><input id="interestBudget" placeholder="Orçamento previsto (MZN)" required><fieldset class="service-selector"><legend>Selecione os serviços que deseja</legend><p>Pode escolher uma ou várias opções.</p><div class="service-choice-grid">' + serviceOptions() + '</div></fieldset><textarea id="interestNotes" rows="3" placeholder="Observações adicionais ou necessidades específicas (opcional)"></textarea><p class="availability-note">A disponibilidade desta data é atualizada pela agenda do proprietário.</p><button class="btn btn--primary" type="submit">Enviar detalhes e iniciar negociação</button></form>' +
    ...
    document.getElementById('interestForm').onsubmit = function (event) { event.preventDefault(); if (!validateReservationDate()) { interestDate.reportValidity(); return; } var selectedServices=Array.from(document.querySelectorAll('input[name="interestServices"]:checked')).map(function(input){return input.value;}); if(!selectedServices.length){window.EspacoOn.toast('Selecione pelo menos um serviço para o evento.');return;} r.eventDetails = { type:document.getElementById('interestType').value.trim(), date:interestDate.value, guests:document.getElementById('interestGuests').value, budget:document.getElementById('interestBudget').value.trim(), services:selectedServices, notes:document.getElementById('interestNotes').value.trim(), sentAt:new Date().toISOString() }; r.negotiationStatus='interested'; r.messages=r.messages||[]; r.messages.push({author:'client',text:'Tenho interesse no espaço. Serviços pretendidos: '+selectedServices.join(', ')+'.',at:new Date().toISOString()}); save(); render(); window.EspacoOn.toast('Interesse enviado. A negociação foi iniciada.'); };
  ...
  function renderReservations() {
    ...
    var reservations = requests.filter(function (r) { return !r.noInterestReason && currentProposals.some(function(p){return p.visitId===r.id&&p.status==='accepted';}); });
    document.getElementById('agendaReservationCount').textContent = reservations.length;
    document.getElementById('agendaVisitCount').textContent = requests.length;
    ...
    container.innerHTML = reservations.map(function (r) { var proposal=currentProposals.find(function(item){return item.visitId===r.id;});var state=proposal.paymentStatus==='paid'?'Confirmada e paga':'Aguardando pagamento';var stateClass=proposal.paymentStatus==='paid'?'badge--ok':'badge--warn';return '<article class="card client-row"><i class="bx bx-building-house"></i><span><strong>' + esc(r.venue) + '</strong><small>Evento em ' + date(r.eventDetails.date) + ' · ' + esc(r.eventDetails.guests) + ' convidado(s)</small></span><span class="badge '+stateClass+'">'+state+'</span></article>'; }).join('');
  }
